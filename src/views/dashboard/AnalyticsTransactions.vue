...
const xName = ref([]);
loginApi.indexOrderGroupMonth().then((res) => {
  xName.value = res.data.map((e) => {
    return e.month + "月";
  });
  series.value.expenses[0].data = res.data.map((e) => {
    series.value.expenses[0].sum += e.value;
    return e.value;
  });
});
      ...
      discrete: [
        {
          ...
          dataPointIndex: series.value["expenses"][0].data.length - 1,
        },
      ],
    },
  };
